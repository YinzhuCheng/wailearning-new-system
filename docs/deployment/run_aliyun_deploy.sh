...
REPO_URL="${REPO_URL:-https://github.com/YinzhuCheng/wailearning-new-system.git}"
BRANCH="${BRANCH:-main}"
...
DB_NAME="${DB_NAME:-wailearning_new}"
DB_USER="${DB_USER:-wailearning_new}"
DB_PASSWORD="${DB_PASSWORD:-}"
INIT_ADMIN_USERNAME="${INIT_ADMIN_USERNAME:-admin}"
INIT_ADMIN_PASSWORD="${INIT_ADMIN_PASSWORD:-}"
INIT_ADMIN_REAL_NAME="${INIT_ADMIN_REAL_NAME:-New System Administrator}"
...
validate_inputs() {
  ...
  assert_replaced "DEFAULT_LLM_API_KEY" "${DEFAULT_LLM_API_KEY}"
...
deploy() {
  ...
  psql_as_postgres -v db_name="${DB_NAME}" -v db_user="${DB_USER}" -v db_password="${DB_PASSWORD}" -f "${RELEASE_DIR}/ops/scripts/init_db.sql"
...
FRONTEND_ADMIN_BASE_URL=${BASE_URL}
...
ENV_EOF
  ...
  (
    cd "${RELEASE_DIR}/apps/web/school"
    cat > .env.production <<ENV_EOF
...
ENV_EOF
  ...
  (
    cd "${RELEASE_DIR}/apps/web/parent"
    cat > .env.production <<ENV_EOF
VITE_API_BASE_URL=${BASE_URL}/api
ENV_EOF
    npm ci --no-audit --no-fund
...
ExecStartPre=${RELEASE_DIR}/.venv/bin/python -m apps.backend.courseeval_backend.bootstrap
