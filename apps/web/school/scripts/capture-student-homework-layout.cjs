...
const outputPath = path.resolve(
  repoRoot,
  process.argv[2] || path.join('pics', 'student-homework-layout-fixed.png')
)
...
async function submitHomework(token, homeworkId, content) {
  return postJson(
    `${apiBase}/api/homeworks/${homeworkId}/submission`,
    {
      content,
      content_format: 'markdown',
      attachment_name: null,
      attachment_url: null,
      declared_used_llm: false
    },
    {
      Authorization: `Bearer ${token}`
    }
...
async function reviewSubmission(token, homeworkId, submissionId, score, comment) {
  const res = await fetch(
    `${apiBase}/api/homeworks/${homeworkId}/submissions/${submissionId}/review`,
    {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
  ...
  if (!res.ok) {
    throw new Error(`PUT review failed ${res.status}: ${await res.text()}`)
  }
  return res.json()
}
...
async function main() {
  ...
  const teacherLogin = await loginApi(scenario.teacher_own.username, scenario.teacher_own.password)
  const studentLogin = await loginApi(scenario.student_plain.username, scenario.student_plain.password)
  const teacherToken = teacherLogin.access_token
  const studentToken = studentLogin.access_token

  const subjects = await getJson(`${apiBase}/api/subjects/course-catalog`, {
    Authorization: `Bearer ${studentToken}`
  })
  const selectedCourse = (subjects || []).find(row => Number(row.id) === Number(scenario.course_required_id))
  if (!selectedCourse) {
    throw new Error(`student-visible required course ${scenario.course_required_id} not found`)
  }

  const stamp = new Date().toISOString().replace(/[:.]/g, '-')
  const first = await createDemoHomework(
    teacherToken,
    scenario,
    `学生端布局演示作业 A ${stamp}`,
  ...
  const second = await createDemoHomework(
    teacherToken,
    scenario,
    `学生端布局演示作业 B ${stamp}`,
  ...
  const submissionA = await submitHomework(studentToken, first.id, '学生端布局截图验证提交 A')
  const submissionB = await submitHomework(studentToken, second.id, '学生端布局截图验证提交 B')
  await reviewSubmission(teacherToken, first.id, submissionA.id, 94, '评分说明需要简洁展示，避免在列表页占据过多垂直空间。')
  await reviewSubmission(teacherToken, second.id, submissionB.id, 88, '第二条评语同样用于验证学生端列表压缩后的显示密度与横向容错。')
  ...
  try {
    ...
    await page.goto(`${uiBase}/login`, { waitUntil: 'domcontentloaded', timeout: 60000 })
    await page.getByTestId('login-username').fill(scenario.student_plain.username)
    await page.getByTestId('login-password').fill(scenario.student_plain.password)
