...
PARENT_WEB_ROOT="${PARENT_WEB_ROOT:-/var/www/courseeval.example/parent}"
...
pushd "${SOURCE_DIR}/apps/web/parent" >/dev/null
npm ci
npm run build
popd >/dev/null

install -d -m 0755 /var/www/certbot
install -d -m 0755 "${PARENT_WEB_ROOT}"
rsync -a --delete "${SOURCE_DIR}/apps/web/parent/dist/" "${PARENT_WEB_ROOT}/"
